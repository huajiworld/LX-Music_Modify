...
export const downloadFile = (url: string, path: string, options: Omit<RNFS.DownloadFileOptions, 'fromUrl' | 'toFile'> = {}) => {
  ...
  background: options.background,    // Continue the download in the background after the app terminates (iOS only)
  discretionary: options.discretionary,  // Allow the OS to control the timing and speed of the download to improve perceived performance  (iOS only)
  cacheable: options.cacheable,      // Whether the download can be stored in the shared NSURLCache (iOS only, defaults to true)
  progressInterval: options.progressInterval,
  progressDivider: options.progressDivider,
  begin: (res: DownloadBeginCallbackResult) => void,
  progress: options.progress, // Callback for download progress
  resumable: options.resumable,    // only supported on iOS yet
  connectionTimeout: options.connectionTimeout, // only supported on Android yet
  readTimeout: options.readTimeout,    // supported on Android and iOS
  backgroundTimeout: options.backgroundTimeout,// Maximum time (in milliseconds) to download an entire resource (iOS only, useful for timing out background downloads)
});
